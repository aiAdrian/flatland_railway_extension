...
- [Rolling Stock](https://github.com/aiAdrian/flatland_railway_extension/blob/master/flatland_extensions/environment_extensions/RollingStock.py)
  ...
  The diagram shows the traction characteristics. On the x-axis the speed is drawn and on the y-axis the tractive effort. The tractive effort (force) is limited by the maximum tractive effort and the maximal power (P/v = F where F = m*a, v : velocity, P = max. power of the traction = RollingStock.v_max_traction * RollingStock.max_traction) 
