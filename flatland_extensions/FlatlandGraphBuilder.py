...
class FlatlandGraphBuilder:
    def __init__(self, railroad_switch_analyser: RailroadSwitchAnalyser):
        ...
        self._full_graph, self.nodes = self._create_full_graph()
        self._reduced_graph, self.nodes = self._create_reduced_graph()
        self._graph = self._full_graph

    def activate_reduced_graph(self):
        self._graph = self._reduced_graph
    ...
    def get_shortest_path(self, start_position, start_direction, target_position):
        ...
        for target_direction in range(4):
            ...
            fn = self.get_graph().nodes.get(from_node, None)
            tn = self.get_graph().nodes.get(to_node, None)
            if fn is not None and tn is not None:
                try:
                    s_path = nx.shortest_path(self.get_graph(), from_node, to_node)
                    for p in s_path:
